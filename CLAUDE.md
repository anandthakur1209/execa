...
- **Mic and system audio streams stay separate end-to-end.** Never pre-mix upstream of the diarizer — per-stream diarization quality (each provider socket sees only one acoustic environment) and the `(meeting_id, source, raw_speaker_id)` data-model distinction both depend on it. Both streams are diarized; mic-stream multi-speaker (in-room hybrid meetings) is a first-class case, not the exceptional path. Spec §4.1, §4.2.
