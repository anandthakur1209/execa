...
- **Mic and system audio streams stay separate end-to-end.** Never pre-mix upstream of any STT provider or diarizer. Per-stream signal isolation is what lets the post-hoc Sarvam batch diarizer (Phase 2 / "Path B") do its job cleanly on each saved WAV, and the `(meeting_id, source, raw_speaker_id)` distinction in the `speakers` table requires the source attribution at insert time. Live streaming under Sarvam collapses each stream to a single label (mic → `displayName`, system → `"Remote"`) because Sarvam streaming has no diarization; multi-speaker labels arrive via the post-hoc batch. Under Deepgram (Phase 6 failover) diarization runs live. Either way, the streams stay separate. Spec §4.1, §4.2, §6.1.
