...
**Phase 3.5c complete (tagged `phase-3.5c`). Ready for Phase 4.** Phase 3.5b manual smoke surfaced a topology-interaction gap: when Sarvam's batch diarization over-segments one voice into multiple system speakers, the mic-side bleed scatters across them and none alone clears the cross-validation thresholds. After the user manually merges the over-segmented system speakers via the sidebar, the dedup state written at swap time stays stale until an explicit Re-run. Phase 3.5c makes the algorithm **merge-aware** (pairwise + concat pre-pass + cross-validation all score against the effective speaker's combined text via `COALESCE(merged_into_speaker_id, id)`) and adds an **auto re-run hook** on merge + split (`AppCoordinator.mergeSpeakers`/`splitSegment` call `DiarizationService.rerunDedupForMeeting` as their final step). `SpeakerBleedDeduper.dedup` is now reset-first — wipes existing audit FKs for the meeting at the top of the function — so re-running on existing state is idempotent. Rename does NOT trigger the hook (label-only, no topology change). The Phase 3.5 design (soft-delete via `transcript_segments.deduped_against_segment_id`, view-layer orphan-speaker filter, two-write swap-then-dedup pattern) and the Phase 3.5b thresholds + pass ordering all carry forward unchanged.

**Phase 3.5b (algorithm, still Active):** Phase 3.5 manual smoke surfaced an algorithmic weakness: Jaccard similarity over raw tokens fails on the most common bleed pattern (mic captures a fragment of a longer system segment — containment is high but Jaccard is low). Phase 3.5b ships a v2 algorithm under a new `bleed_dedup_algorithm_version` settings key (default `"v2"`, v1 retained as flag-fallback): containment coefficient (`|mic ∩ system| / |mic|`) ≥ 0.75 over Porter-light-stemmed tokens; ASCII-only stemmer (Devanagari pass-through); concatenation pre-pass that scores consecutive same-speaker mic fragments inside one containing system segment as one unit; cross-validation post-pass that promotes the remaining unflagged segments of any mic speaker who's already ≥ 80% flagged with ≥ 3 absolute flagged segments. The Phase 3.5 design (soft-delete via `transcript_segments.deduped_against_segment_id`, view-layer orphan-speaker filter via `SpeakerQueries.visibleSpeakers`, two-write swap-then-dedup pattern) is unchanged — v2 swaps only the scoring function and adds two passes.
...
## Phase 3.5c — Merge-aware dedup + auto re-run on topology change

**Goal:** Phase 3.5b's swap-time dedup state goes stale when the user later merges over-segmented system speakers via the sidebar — the cross-validation cumulative-containment math, written against the unmerged topology, can't catch bleed that was scattered across the now-merged speakers, and visible duplicates persist until an explicit Re-run. Phase 3.5c makes the algorithm merge-aware (all passes score against the effective speaker's combined text) and adds an auto re-run hook so merge + split immediately heal the dedup state. Reset-first semantics make the algorithm idempotent under re-run.

**Scope:**
- `Segment` gains `effectiveSpeakerID: Int64` loaded via JOIN+COALESCE on `speakers.merged_into_speaker_id`. New `EffectiveSystemSpeaker` aggregate (combined text, time envelope, total coverage) built by `buildEffectiveSystemSpeakers(systems:)`.
- Pairwise pass, concat pre-pass, and cross-validation all iterate `[EffectiveSystemSpeaker]` and score against `combinedText`. Audit FK anchors on `bestOverlapSegment` (constituent segment with highest time-overlap fraction; tie → longest, then lowest id) so the durable DB pointer survives a subsequent unmerge/re-split.
- `SpeakerBleedDeduper.dedup` becomes reset-first: writes `UPDATE transcript_segments SET deduped_against_segment_id = NULL WHERE meeting_id = ?` at the top of the function. `loadSegments`'s `WHERE deduped_against_segment_id IS NULL` filter removed — every final segment is loaded regardless of FK state.
- `DiarizationService.runDedupPass` renamed to `rerunDedupForMeeting` (now internal). Inline post-swap call site unchanged in behavior; the rename reflects that it's also reachable from the topology-change hook.
- `AppCoordinator.mergeSpeakers` and `splitSegment` invoke `rerunDedupForMeeting` as their final step (after the topology mutation + live `TranscriptStore` propagation). `renameSpeaker` is unchanged — rename is label-only and doesn't affect topology.
- `AppCoordinator.init` accepts an optional `database:` parameter for test injection; production call sites keep their zero-arg shape via default `nil`.
- Single-hop alias resolution (`COALESCE(merged_into_speaker_id, id)`) is intentional — matches the Phase 3 invariant that the UI merges A → B only when neither A nor B is itself an alias. If a future workflow lets aliases chain, the JOIN would need a recursive CTE.
- Code shape: v2 algorithm body in `SpeakerBleedDedupV2.swift`; scoring primitives split into `SpeakerBleedDedupScoring.swift`; cross-validation into `SpeakerBleedDedupV2CrossValidation.swift` (under the file-length cap).

**Acceptance:**
- Case A regression: 1 mic segment whose tokens are split across 2 over-segmented system speakers (each below containment threshold alone). Pre-merge: not flagged. After user merges sys2 into sys1: rerun fires, mic flags against the combined effective speaker. End-to-end test pinned.
- Cross-val promotion reversibility: 5-mic-segment speaker with 4 pairwise + 1 cross-val-promoted segment. User splits the promoted segment off. Singleton new speaker → cross-val doesn't satisfy ≥ 3 minimum → promoted segment loses its FK on the re-run.
- Rename does not trigger re-run: a deliberately wrong audit FK seeded pre-rename survives the rename unchanged (re-run would have wiped + re-derived it).
- Reset-first heals stale state: wrong FK seeded pre-re-run gets wiped, then re-derived to the correct one against the current topology.
- Paraphrase guard survives merged topology: legitimately different word choice across merged system speakers stays not-flagged.
- Swap-time behavior unchanged when no merges exist: the reset at the top of `dedup` is a no-op (fresh rows are already NULL); algorithm output identical to Phase 3.5b's swap-time output.
